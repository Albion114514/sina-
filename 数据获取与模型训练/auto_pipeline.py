...
def find_latest_artifacts() -> Tuple[Optional[str], Optional[str]]:
    """
    返回 (best_model_path, vocab_json_path)，优先使用 models/latest/
    若不存在，则在 train_output/*/model_output/ 里按 mtime 逆序扫描。
    """
    ...
    for p in Path("train_output").glob("*/model_output/best_model.pth"):
        candidates.append(p)
    if not candidates:
        return None, None
    ...
    return (str(best), str(vocab)) if vocab.exists() else (str(best), None)


def print_info_line(s: str):
    print(f"[pipeline] {s}")
...
def run_algorithm(mode: str, rumor_type: Optional[int], outdir: Path) -> Tuple[bool, Optional[float]]:
    ...
    if mode == "train":
        ...
        # 注意：你现有版本可能已经在上游生成 labels.csv，这里保持兼容（labels_csv 在其它位置构建）。
        ...
        if prev_model:
            print_info_line(f"Reusing pretrained model: {prev_model}")
            cmd += ["--pretrained-path", prev_model]
    else:
        # 测试：使用“最新模型”进行推理，输出 prediction_result.csv
        # 这里直接在 pipeline 内实现一次性推理，避免依赖外部脚本 CLI。
        prev_model, prev_vocab = find_latest_artifacts()
        if not prev_model or not prev_vocab:
            print_warning("未找到可用的最新模型（models/latest/ 或 train_output/*/model_output/）。")
            return False, None
        # 用内联 Python 执行推理（复用 rumor_train.py 中的网络结构）
        py = r"""
import json, sys, torch, pandas as pd, jieba, csv
...
class InferDS(Dataset):
    def __init__(self, content_path, comment_path, vocab, max_len=128, max_comments=10):
        self.vocab = vocab; self.max_len=max_len; self.max_comments=max_comments
        self.cdf = pd.read_csv(content_path, header=None)
        self.kdf = pd.read_csv(comment_path, header=None)
        # 保存 id 列，便于写回
...
def col_fn(batch):
    ...
    pad=lambda t: torch.cat([t, torch.zeros(max_seq-len(t), dtype=torch.long)],0) if len(t)<max_seq else t
    ...
    for cmts in cmts_list:
        ...
        Cm.append(torch.stack([pad(t) for t in cmts],0))
    M = torch.stack(Cm,0)
    return ids, C, M

ds = InferDS(content_path, comment_path, vocab)
dl = DataLoader(ds, batch_size=32, shuffle=False, collate_fn=col_fn)
device = "cuda" if torch.cuda.is_available() else "cpu"
model = RumorDetector(vocab_size=len(vocab), pad_idx=PAD).to(device)
...
"""
        cmd = [sys.executable, "-c", py,
               str(content), str(comments),
               str(prev_vocab), str(prev_model),
               str(output)]
...
def main():
    ...
    try:
        ...
        if not os.path.exists(labels_csv):
            if choices.get('mode') == 'train' and choices.get('rumor_type') in (1, 2):
                ...
                try:
                    import csv
                    # 正确读取微博ID
                    with open(os.path.join(export_dir, 'weibo_data_format.csv'), 'r', encoding='utf-8') as f:
                        reader = csv.reader(f)
                        rows = list(reader)

                    # 生成标签文件
                    with open(auto_labels_path, 'w', newline='', encoding='utf-8') as g:
                        writer = csv.writer(g)
                        # 1=谣言→标签1；2=非谣言→标签0
                        label_value = 1 if choices['rumor_type'] == 1 else 0
                        for row in rows:
                            if row:  # 确保行不为空
                                content_id = row[0].strip()  # 第一列是content_id
                                writer.writerow([content_id, label_value])

                    labels_csv = auto_labels_path
                    print(f"[pipeline] 自动生成标签文件: {labels_csv}")
                    print(
                        f"[pipeline] 为 {len(rows)} 条微博生成标签: {label_value} ({'谣言' if label_value == 1 else '非谣言'})")
                    do_training = True
                except Exception as e:
                    print(f"[WARN] 自动生成标签失败: {e}")
                    do_training = False
            else:
                print(f"[WARN] 标签CSV文件不存在: {labels_csv}。请提供 --labels-csv；训练将被跳过。")
                do_training = False
        else:
            print(f"[pipeline] 使用现有标签文件: {labels_csv}")
            do_training = True
        ...
        if choices['mode'] == 'train':
            # 训练模式：需要标签文件
            if not os.path.exists(labels_csv):
                if choices.get('rumor_type') in (1, 2):
                    auto_labels_path = os.path.join(export_dir, 'labels.csv')
                    try:
                        import csv
                        # 正确读取微博ID
        ...
        # 测试阶段：调用推理算法
        elif choices['mode'] == 'test':
            print_step(8, "运行测试模式推理")

            # 使用原有的 run_algorithm 函数进行推理
            success, acc = run_algorithm(choices['mode'], choices.get('rumor_type'), outdir)

            if success:
                print_success("测试推理完成")
                if acc is not None:
                    print(f"[pipeline] 推理完成，准确率: {acc:.6f}")
            else:
                print_error("测试推理失败")
                # 询问是否继续
                if not ARGS.non_interactive:
                    yn = input("推理失败，是否继续后续步骤? (y/n): ").strip().lower()
                    if yn != "y":
                        sys.exit(1)
